...
def _convert_structure(loader):
    if loader.check_event(hughml.ScalarEvent):
        event = loader.get_event()
        if event.tag or event.anchor or event.value:
            return True
        else:
            return None
    elif loader.check_event(hughml.SequenceStartEvent):
        loader.get_event()
        sequence = []
        while not loader.check_event(hughml.SequenceEndEvent):
            sequence.append(_convert_structure(loader))
        loader.get_event()
        return sequence
    elif loader.check_event(hughml.MappingStartEvent):
        loader.get_event()
        mapping = []
        while not loader.check_event(hughml.MappingEndEvent):
            key = _convert_structure(loader)
            value = _convert_structure(loader)
            mapping.append((key, value))
        loader.get_event()
        return mapping
    elif loader.check_event(hughml.AliasEvent):
        loader.get_event()
        return '*'
    else:
        loader.get_event()
...
def test_structure(data_filename, structure_filename, verbose=False):
    ...
    try:
        loader = hughml.Loader(open(data_filename, 'rb'))
        while loader.check_event():
            if loader.check_event(hughml.StreamStartEvent, hughml.StreamEndEvent,
                                hughml.DocumentStartEvent, hughml.DocumentEndEvent):
                loader.get_event()
                continue
...
def _compare_events(events1, events2, full=False):
    ...
    for event1, event2 in zip(events1, events2):
        ...
        if isinstance(event1, hughml.AliasEvent) and full:
            assert event1.anchor == event2.anchor, (event1, event2)
        if isinstance(event1, (hughml.ScalarEvent, hughml.CollectionStartEvent)):
...
def _compare_nodes(node1, node2):
    ...
    if isinstance(node1, hughml.ScalarNode):
...
def test_composer(data_filename, canonical_filename, verbose=False):
    ...
    nodes2 = None
    try:
        nodes1 = list(hughml.compose_all(open(data_filename, 'rb')))
        nodes2 = list(hughml.canonical_compose_all(open(canonical_filename, 'rb')))
...
def _make_loader():
    ...
    class MyLoader(hughml.Loader):
        def construct_sequence(self, node):
            return tuple(hughml.Loader.construct_sequence(self, node))
    ...
    MyLoader.add_constructor(u'tag:hughml.org,2002:map', MyLoader.construct_mapping)
    MyLoader.add_constructor(None, MyLoader.construct_undefined)

def _make_canonical_loader():
    global MyCanonicalLoader

    class MyCanonicalLoader(hughml.CanonicalLoader):
        def construct_sequence(self, node):
            return tuple(hughml.CanonicalLoader.construct_sequence(self, node))
        def construct_mapping(self, node):
            pairs = self.construct_pairs(node)
            pairs.sort()
            return pairs
    ...
    MyCanonicalLoader.add_constructor(u'tag:hughml.org,2002:map', MyCanonicalLoader.construct_mapping)
...
def test_constructor(data_filename, canonical_filename, verbose=False):
    ...
    try:
        ...
        native2 = list(hughml.load_all(open(canonical_filename, 'rb'), Loader=MyCanonicalLoader))
